      ...
      Utils.safe_replace_file(server_dir.monit_conf) do |f|
        ...
        server_dir.apps.each do |app|
          ...
          if app.pid_file && app.start_cmd
            ...
            f.puts "  start program = \"/bin/bash -c '#{app.start_cmd}'\""
            f.puts "  stop program = \"/bin/bash -c '#{app.stop_cmd}'\"" if app.stop_cmd
            ...
            f.puts "  if changed checksum then exec \"/bin/bash -c '#{app.restart_cmd}'\""
          end
        end
      end
    end
